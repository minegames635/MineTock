...
class CloudinaryError extends Error { constructor(message,status,body,provider){super(message);this.name='CloudinaryError';this.status=status;this.body=body;this.provider=provider} }
function cloudProviders(env){return [{cloudName:env.CLOUDINARY_CLOUD_NAME,apiKey:env.CLOUDINARY_API_KEY,apiSecret:env.CLOUDINARY_API_SECRET},{cloudName:env.CLOUDINARY_SECONDARY_CLOUD_NAME,apiKey:env.CLOUDINARY_SECONDARY_API_KEY,apiSecret:env.CLOUDINARY_SECONDARY_API_SECRET}].filter(p=>p.cloudName&&p.apiKey&&p.apiSecret)}
async function cloudWithCredentials(provider,action,params,file,resource='video'){const form=new FormData();for(const [k,v] of Object.entries(params))form.set(k,String(v));form.set('api_key',provider.apiKey);form.set('signature',await signature(params,provider.apiSecret));if(file)form.set('file',file);const response=await fetch('https://api.cloudinary.com/v1_1/'+provider.cloudName+'/'+resource+'/'+action,{method:'POST',body:form});const body=await response.json().catch(()=>({}));if(!response.ok){const error=new CloudinaryError(body?.error?.message||'Хранилище не приняло файл.',response.status,body,provider);throw error}return body}
function isCloudCapacityError(error){const message=String(error?.body?.error?.message||error?.message||'').toLowerCase();return [402,420,429,507].includes(error?.status)||/(quota|limit|bandwidth|credits|usage|rate.?limit|resource|capacity|too many|free plan)/i.test(message)}
async function uploadWithFallback(env,params,file,resource='video'){const list=cloudProviders(env);if(!list.length)throw Error('Хранилище ещё не подключено');let last;for(let i=0;i<list.length;i++){try{const result=await cloudWithCredentials(list[i],'upload',params,file,resource);return {...result,cloudName:list[i].cloudName}}catch(error){last=error;if(i===list.length-1||!isCloudCapacityError(error))throw error}}throw last}
...
export default {async fetch(request,env){
 ...
 try {
   if(!cloudProviders(env).length)return json({error:'Хранилище ещё не подключено'},503,cors);
  ...
  // Photos are rate-limited to protect the free image tier. Video publishing
  // has no artificial daily cap; Cloudinary and the per-file limits remain the
  // safety boundary for the release.
  if(imageUpload){
   const gate=env.UPLOAD_GATE.get(env.UPLOAD_GATE.idFromName(user.sub));
   const permit=await gate.fetch('https://internal/image',{method:'POST'});
   if(!permit.ok)return json({error:'Лимит: 30 изображений в сутки'},429,cors);
  }
  ...
  if(imageUpload) {
   ...
   const result=await uploadWithFallback(env,{public_id:publicId,timestamp:Math.floor(Date.now()/1000),overwrite:false,transformation:'c_limit,w_1024,h_1024',format:'webp'},file,'image');
   return json({url:result.secure_url,publicId:result.public_id,cloudName:result.cloudName},200,cors);
  }
  ...
  const publicId=`minetock/${user.sub}/${id}`;
   const result=await uploadWithFallback(env,{public_id:publicId,timestamp:Math.floor(Date.now()/1000),overwrite:false},file);
  if(!result.duration||result.duration>180||result.bytes>MAX||!['mp4','mov','webm','mkv'].includes(result.format)){
    await cloudWithCredentials(cloudProviders(env).find(p=>p.cloudName===result.cloudName)||cloudProviders(env)[0],'destroy',{public_id:publicId,timestamp:Math.floor(Date.now()/1000),invalidate:true});return json({error:'Видео должно быть короче 3 минут, до 50 МБ, в формате MP4/MOV/WebM'},400,cors)
  }
   return json({url:result.secure_url,publicId:result.public_id,cloudName:result.cloudName},200,cors);
...
export class UploadGate {
 ...
 async fetch(request){const day=Math.floor(Date.now()/86400000);const permitted=await this.ctx.storage.transaction(async tx=>{const state=await tx.get('images');const count=state?.day===day?state.count:0;if(count>=30)return false;await tx.put('images',{day,count:count+1});return true});return new Response(null,{status:permitted?204:429})}
}
