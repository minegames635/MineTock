...
export default {async fetch(request,env){
 ...
 if(request.method!=='POST'||new URL(request.url).pathname!=='/upload')return json({error:'Not found'},404,cors);
 try {
  ...
  const size=Number(request.headers.get('Content-Length'));if(!Number.isFinite(size)||size>MAX+65536)return json({error:'Максимум 50 МБ'},413,cors);
  const gate=env.UPLOAD_GATE.get(env.UPLOAD_GATE.idFromName(user.sub));const permit=await gate.fetch('https://internal/reserve',{method:'POST'});
  if(!permit.ok)return json({error:'Лимит: 3 попытки загрузки в сутки. Попробуйте завтра (UTC).'},429,cors);
  ...
  while(true){const {done,value}=await reader.read();if(done)break;total+=value.byteLength;if(total>MAX+65536){await reader.cancel();return json({error:'Максимум 50 МБ'},413,cors)}chunks.push(value)}
 ...
 }catch(error){console.error('Upload failed:',error.message);return json({error:'Не удалось загрузить видео. Проверьте настройки хранилища и формат файла.'},500,cors)}
}};
export class UploadGate {
 constructor(ctx){this.ctx=ctx}
 async fetch(){const day=Math.floor(Date.now()/86400000);const permitted=await this.ctx.storage.transaction(async tx=>{const state=await tx.get('quota');const count=state?.day===day?state.count:0;if(count>=3)return false;await tx.put('quota',{day,count:count+1});return true});return new Response(null,{status:permitted?204:429})}
}
